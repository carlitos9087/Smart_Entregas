...
class Mapa(QWidget):
    def __init__(self):
        ...
        self.status_label = QLabel(f"Status: inativo", self)
    ...
    def adicionar_obstaculo(self):
        ...
        try:
            ...
            print(self.rotas_bloqueadas, "rotas bloqueadas")
            ...
            if len(rota_coordenadas) >= 2:
                ...
                # Muda a cor da linha entre os dois primeiros pontos da rota para vermelho
                if (rota_ids[0], rota_ids[1]) in self.linhas_rotas:
                    ...
                    # Remove a linha do dicionário de rotas
                    del self.linhas_rotas[(rota_ids[0], rota_ids[1])]
                    ...
                    # Remove a linha do dicionário de rotas
                    del self.linhas_rotas[(rota_ids[1], rota_ids[0])]
    ...
    def remover_obstaculos(self):
        ...
        for linha in self.linhas_rotas.values():
            linha.setPen(QPen(Qt.black, 2))
    ...
    def iniciar_rota(self):
        """Inicia o carro na rota definida pelo usuário."""
        rota_texto = self.rota_input.text()
        try:
            # Converte o texto da rota para uma lista de IDs
            rota_ids = eval(rota_texto)

            # Verifica se a rota contém algum obstáculo
            if any((id1, id2) in self.rotas_com_obstaculos or (id2, id1) in self.rotas_com_obstaculos for id1, id2 in zip(rota_ids[:-1], rota_ids[1:])):
                print("A rota contém obstáculos. Selecione uma nova rota.")
                return

            # Inicia a animação do carro
            self.carro.rota = rota_ids
            ...
            self.velocidade_label.setText("Velocidade: 1")
